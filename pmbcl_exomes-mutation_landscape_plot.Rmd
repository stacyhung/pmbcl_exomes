...
mutations.maf <- read.maf("/Volumes/shung/projects/PMBCL_exomes/Mutsig/mutsig_input/mutation_file/maftools/paired_and_unpaired.snvs_and_indels.sig_genes_v3.hugo_gene.AF_filtered.gene_filtered.with_VAF.maf", 
                          clinicalData = clinData.df)

# OR mutations with additional copy number data:
mutations.maf <- read.maf("/Volumes/shung/projects/PMBCL_exomes/Mutsig/mutsig_input/mutation_file/maftools/paired_and_unpaired.snvs_and_indels.sig_genes_v3.hugo_gene.AF_filtered.gene_filtered.with_VAF.maf", 
                          clinicalData = clinData.df,
...
# significant mutations *plus* IRF (use this mutation list for highlighting IRF pathway genes)
...
#mutations.maf <- read.maf("/Volumes/shung/projects/PMBCL_exomes/Mutsig/mutsig_input/mutation_file/maftools/paired_and_unpaired.snvs_and_indels.sig_genes_v3.hugo_gene.AF_filtered.gene_filtered.maf")

# MAF Summary Plot
#plotmafSummary(maf = mutations.maf, rmOutlier = TRUE, dashboard = TRUE)

# Oncoplot
#oncoplot(maf = mutations.maf, genes = c("SOCS1", "STAT6", "GNA13", "ITPKB", "ACTB", "TNFAIP3", "IL4R", "ZNF217", "NFKBIE", "CIITA", "IRF2BP2", "CD58"))
...
sig.genes <- c("SOCS1", "HIST1H3D", "TNFAIP3", "EZH2")
...
oncoplot(maf = mutations.maf, genes = c("CISH", "IL4R", "IL13RA1", "STAT6", "SOCS1", "PTPN1"))
...
somaticInteractions(maf = mutations.maf, genes=sig.genes, top = 50, pvalue = c(0.05, 0.1))

# oncoplots for EACH mutually exclusive / co-occurring interaction (most significant):

# mutually exclusive interactions:
plot.me1 <- oncoplot(maf = mutations.maf, genes = c("GNA13", "IL4R"), removeNonMutated = FALSE)
plot.me2 <- oncoplot(maf = mutations.maf, genes = c("CISH", "STAT6"), removeNonMutated = FALSE)
plot.me3 <- oncoplot(maf = mutations.maf, genes = c("CXCR5", "SOCS1"), removeNonMutated = FALSE)
# co-occurring interactions:
oncoplot(maf = mutations.maf, genes = c("CISH", "ZFP36L1"), removeNonMutated = FALSE)
oncoplot(maf = mutations.maf, genes = c("IRF2BP2", "ACTB"), removeNonMutated = FALSE)
...
# Plotting VAF
vafPlot <- plotVaf(maf = mutations.maf, vafCol = 'VAF', flip = TRUE, top = 30, showN = TRUE)
```
...
irf.genes <- c("IRF2BP2", "IRF8", "IRF4", "CISH", "IL13RA1", "IL1RAPL2") #IRF1 "IFNL3" protein structure not found 
...
# omitting IGLJ2
irf.genes = c("IRF2BP2", "IRF8", "IRF4", "IRF1", "CISH", "IL13RA1", "IL1RAPL2", "IFNL3")
